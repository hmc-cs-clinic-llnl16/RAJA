                    ...
                    const RAJAVec<Index_type>& is_indices)
{
  ...
  cout << "\n END RAJA::forall tests: ibuild = " << ibuild << endl;
}
                           ...
                           const RAJAVec<Index_type>& is_indices)
{
  ...
  runBasicForall_IcountTest<IndexSet::ExecPolicy<seq_segit, seq_exec> >(
      "ExecPolicy<seq_segit, seq_exec>", in_array, alen, iset, is_indices);

  runBasicForall_IcountTest<IndexSet::ExecPolicy<seq_segit, simd_exec> >(
      "ExecPolicy<seq_segit, simd_exec>", in_array, alen, iset, is_indices);
  ...
  runBasicForall_IcountTest<IndexSet::ExecPolicy<omp_parallel_for_segit,
                                                 seq_exec> >(
      "ExecPolicy<omp_parallel_for_segit, seq_exec>",
      in_array,
      alen,
  ...
  runBasicForall_IcountTest<IndexSet::ExecPolicy<cilk_for_segit, seq_exec> >(
      "ExecPolicy<cilk_for_segit, seq_exec>", in_array, alen, iset, is_indices);

  runBasicForall_IcountTest<IndexSet::ExecPolicy<cilk_for_segit, simd_exec> >(
      "ExecPolicy<cilk_for_segit, simd_exec>",
      in_array,
      alen,
      iset,
  ...
  cout << "\n END RAJA::forall_Icount tests: ibuild = " << ibuild << endl;
}
...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv))
{
  ...
  for (unsigned ibuild = 1; ibuild < NumBuildMethods; ++ibuild) {
    ...
    if (index[ibuild] == index[0]) {
      ...
      cout << "\tIndexSet " << ibuild << " DOES NOT MATCH IndexSet 0!! "
           << endl;
    }
    ...
    if (index[ibuild] != index[0]) {
      cout << "\tIndexSet " << ibuild << " MATCHES IndexSet 0!! " << endl;
    } else {
      s_ntests_passed++;
      s_ntests_passed_total++;
    }
  ...
  for (unsigned ibuild = 0; ibuild < run_tests; ++ibuild) {
    runForallTests(ibuild, parent, array_length, index[ibuild], is_indices);
  }

  for (unsigned ibuild = 0; ibuild < run_tests; ++ibuild) {
    runForall_IcountTests(
        ibuild, parent, array_length, index[ibuild], is_indices);
  }
  ...
  getIndicesConditional(even_indices, index[0], [](Index_type idx) {
    return !(idx % 2);
  });
  ...
  getIndicesConditional(lt300_indices, index[0], [](Index_type idx) {
    return (idx < 300);
  });
